...
/*int main() {
//    Elf_obj * e = elf_open("../bblvmlinuxinitramfs2");

    Elf_obj * e = elf_open("../install/riscv64-unknown-elf/share/riscv-tests/isa/rv64ui-p-addi");


    char * fromhost = "fromhost";
    char * tohost = "tohost";
    ...
    while (curr_sym) {
        ...
        if (strcmp(fromhost, symname) == 0) {
            ...
            printf("fromhost size: %ld\n", curr_sym->st_size);
        } else if (strcmp(tohost, symname) == 0) {
            // get tohost addr
            tohost_addr = curr_sym->st_value;
            printf("tohost size: %ld\n", curr_sym->st_size);

        }
        curr_sym = elf_nextsym(e, curr_sym);
    }
    printf("fromhost addr 0x%016lx\n", fromhost_addr);
    printf("tohost addr 0x%016lx\n", tohost_addr);
