...
static int riscv_gdb_get_fpu(CPURISCVState *env, uint8_t *mem_buf, int n)
{
    if (n < 32) {
        ...
        target_ulong val = 0;
        if (riscv_csrrw(env, CSR_FFLAGS, &val, 0, 0) == 0) {
            return gdb_get_regl(mem_buf, val);
        }
    }
    else if (n == 33) {
        target_ulong val = 0;
        if (riscv_csrrw(env, CSR_FRM, &val, 0, 0) == 0) {
            return gdb_get_regl(mem_buf, val);
        }
    }
    else if (n == 34) {
        target_ulong val = 0;
        if (riscv_csrrw(env, CSR_FCSR, &val, 0, 0) == 0) {
            return gdb_get_regl(mem_buf, val);
        }
    }
    return 0;
...
static int riscv_gdb_set_fpu(CPURISCVState *env, uint8_t *mem_buf, int n)
{
    if (n < 32) {
        ...
        if (riscv_csrrw(env, CSR_FFLAGS, NULL, val, -1) == 0) {
            return sizeof(target_ulong);
        }
    }
    else if (n == 33) {
        target_ulong val = ldtul_p(mem_buf);
        if (riscv_csrrw(env, CSR_FRM, NULL, val, -1) == 0) {
            return sizeof(target_ulong);
        }
    }
    else if (n == 34) {
        target_ulong val = ldtul_p(mem_buf);
        if (riscv_csrrw(env, CSR_FCSR, NULL, val, -1) == 0) {
            return sizeof(target_ulong);
        }
    }
    return 0;
...
static int riscv_gdb_get_csr(CPURISCVState *env, uint8_t *mem_buf, int n)
{
    if (n < 4096) {
        ...
        if (n < ARRAY_SIZE(csr_register_map) &&
            riscv_csrrw(env, csr_register_map[n], &val, 0, 0) == 0) {
            return gdb_get_regl(mem_buf, val);
        }
    }
    return 0;
...
static int riscv_gdb_set_csr(CPURISCVState *env, uint8_t *mem_buf, int n)
{
    if (n < 4096) {
        target_ulong val = ldtul_p(mem_buf);
        if (n < ARRAY_SIZE(csr_register_map) &&
            riscv_csrrw(env, n, NULL, val, -1) == 0) {
            return sizeof(target_ulong);
        }
    }
    return 0;
