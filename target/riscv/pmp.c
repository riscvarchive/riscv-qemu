                   ...
                   target_ulong page_size, target_ulong *tlb_entry_size)
{
    ...
    *tlb_entry_size = page_size;
    ...
    for (size_t i = 0; i < env->pmp_state.active_rules; i++)
    {
        ...
        case PMP_AMATCH_NA4:
            /* The current spec wording for PMP granularity detection requires
             * a terminal bit for granule size spans; generalize NA4 -> NAG
             * (Naturally Aligned Granule) */
            sa = addr_reg << 2;
        ...
        trace_pmp_rule_match(env->mhartid, addr, access_priv, sa, ea, cfg_reg,
                             result);
        if (result) {
